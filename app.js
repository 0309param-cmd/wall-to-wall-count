...
let sessionScanned = 0;  // lines added (scanned) this session
let sessionUploaded = 0; // lines actually confirmed saved to the database
...
async function loadSkuMaster() {
  try {
    // Cache-busting query param defeats any CDN-level caching (GitHub Pages'
    // CDN can serve a stale copy for a few minutes after an update,
    // independent of browser cache / service worker settings).
    const res = await fetch(`sku-master.json?v=${Date.now()}`, { cache: "no-store" });
    skuMaster = await res.json();
    showToast(`Loaded ${skuMaster.length} SKUs`);
  } catch (e) {
    console.error("Could not load sku-master.json", e);
    showToast("Could not load SKU master list — check your connection", true);
  }
}
...
skuSearch.addEventListener("input", () => {
  ...
  const allMatches = skuMaster.filter((s) =>
  ...
  ).join("") + (allMatches.length > matches.length
    ? `<div class="ac-more">+ ${allMatches.length - matches.length} more match — type more to narrow it down</div>`
    : "");
...
function renderSessionStats() {
  $("sessionScanned").textContent = sessionScanned;
  $("sessionUploaded").textContent = sessionUploaded;
  const bar = document.querySelector(".session-stats");
  const check = $("sessionCheck");
  if (sessionScanned === sessionUploaded) {
    bar.classList.remove("mismatch");
    check.textContent = sessionScanned > 0 ? "✓" : "";
  } else {
    bar.classList.add("mismatch");
    check.textContent = `⚠ ${sessionScanned - sessionUploaded} line(s) not yet saved`;
  }
}
...
$("submitBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    sessionUploaded += pendingLines.length;
    renderSessionStats();
    ...
    renderSessionStats(); // reflects the gap between scanned and uploaded
  } finally {
    btn.disabled = false;
